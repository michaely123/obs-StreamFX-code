...
transform_instance::transform_instance(obs_data_t* data, obs_source_t* context) : obs::source_instance(data, context), _gfx_util(::streamfx::gfx::util::get()), _standard_effect(), _transform_effect(), _source_size(), _mip_levels(), _mesh(), _sampler(), _mipmapper(), _final_rt(), _final_tex(), _mesh_dirty(true), _is_rendered(false), _cache_size(), _camera_mode(), _camera_fov(), _params(), _corners(), _mipmap_enabled()
{
	{
		auto gctx = obs::gs::context();

		_final_rt = std::make_shared<streamfx::obs::gs::texrender>(GS_RGBA, GS_ZS_NONE);
		_mesh     = std::make_shared<streamfx::obs::gs::vertexbuffer>(uint32_t(4u), uint8_t(1u));
		{
		...
		{
			gs_sampler_info si;
			si.address_u = si.address_v = si.address_w = GS_ADDRESS_CLAMP;
			si.filter                                  = GS_FILTER_ANISOTROPIC;
			si.max_anisotropy                          = 16;
			_sampler                                   = streamfx::obs::gs::sampler::pool::instance()->acquire(si);
		}
...
#define SET_IF_UNSET(TYPE, NAME, value)             \
	...
	if (version < STREAMFX_MAKE_VERSION(0, 8, 0, 0)) {
		if (obs_data_has_user_value(settings, "Filter.Transform.Position.X")) {
			obs_data_set_double(settings, "Filter.Transform.Position.X", -obs_data_get_double(settings, "Filter.Transform.Position.X"));
		}
		if (obs_data_has_user_value(settings, "Filter.Transform.Position.Y")) {
			obs_data_set_double(settings, "Filter.Transform.Position.Y", -obs_data_get_double(settings, "Filter.Transform.Position.Y"));
		}
	}
...
{
	// Retrieve the source's current size.
	std::pair<size_t, size_t> source_size;
	if (auto target = _self.get_filter_target(); target) {
		source_size = {target.width(), target.height()};
	}

	// If the source changed size, update a few things.
	if (_mesh_dirty || (_source_size.first != source_size.first) || (_source_size.second != source_size.second)) {
		// Update the stored size.
		_source_size = source_size;

		// Update the cache size.
		if (_mipmap_enabled) {
			double_t aspect    = double_t(_source_size.first) / double_t(_source_size.second);
			double_t aspect2   = 1.0 / aspect;
			_cache_size.first  = std::clamp(uint32_t(pow(2, streamfx::util::math::get_power_of_two_exponent_ceil(_cache_size.first))), 1u, 8192u);
			_cache_size.second = std::clamp(uint32_t(pow(2, streamfx::util::math::get_power_of_two_exponent_ceil(_cache_size.second))), 1u, 8192u);

			if (aspect > 1.0) {
				_cache_size.second = std::clamp<uint64_t>(streamfx::util::math::pow<uint64_t>(2, streamfx::util::math::get_power_of_two_exponent_ceil(static_cast<uint64_t>(_cache_size.first * aspect2))), 1u, 8192u);
			} else if (aspect < 1.0) {
				_cache_size.first = std::clamp(uint32_t(pow(2, streamfx::util::math::get_power_of_two_exponent_ceil(uint64_t(_cache_size.second * aspect)))), 1u, 8192u);
			} // This transparently handles aspect == 1.0, since we already did the math for that above.

			// Update the number of mip levels.
			_mip_levels = _mipmapper.calculate_max_mip_level(_cache_size.first, _cache_size.second);
		} else {
			_cache_size = source_size;
		}

		// Mark mesh as dirty if not using corner pin.
	...
	// Update the mesh.
	if (_mesh_dirty && (_camera_mode != transform_mode::CORNER_PIN)) {
		// Calculate Aspect Ratio
		float aspect_ratio_x = float(_source_size.first) / float(_source_size.second);
...
{
	// Resource acquisition and time
	// - Render the original source into a rendertarget, acquiring [Cache].
	// - If Mip-Mapping is enabled:
	//   - Generate a Mip-Map of [Cache], acquiring [Mip-Map]
	//   - Release [Cache]
	// - Transform [Cache] or [Mip-Map], depending on what's remaining, acquiring [Final]
	// - Release [Cache] and [Mip-Map].
	//
	// Temporary RTs:
	// - [Cache]
	// - [Mip-Map]
	...
	// Test for our own sanity.
	if (!(_self.get_filter_target() && _self.get_filter_parent() && effect && _source_size.first && _source_size.second && _cache_size.first && _cache_size.second)) {
		// We're clearly insane, so we should not render anything.
		obs_source_skip_video_filter(_self);
		return;
	}

	if (!_is_rendered) {
		// Acquire a render target for caching.
		auto cache_rt = streamfx::obs::gs::texrender::pool::instance()->acquire(GS_RGBA, GS_ZS_NONE);
		{
			auto op = cache_rt->render(_cache_size.first, _cache_size.second);
			gs_ortho(0, static_cast<float>(_source_size.first), 0, static_cast<float>(_source_size.second), -1., 1.);
			gs_clear(GS_CLEAR_COLOR, &clear_color, 0, 0);
			if (_self.process_filter_begin(GS_RGBA, OBS_ALLOW_DIRECT_RENDERING)) {
				gs_blend_state_push();
				...
				_self.process_filter_end(default_effect, _source_size.first, _source_size.second);

				gs_blend_state_pop();
			} else { // Cleanly exit the render call.
				op.reset();
				obs_source_skip_video_filter(_self);
				return;
			}
		...
		// Retrieve the texture from the previous operation.
		auto buffer_tex = cache_rt->get_texture();
		if (!buffer_tex) {
			obs_source_skip_video_filter(_self);
			return;
		}

		// If requested, generate a mip-map for later rendering.
		if (_mipmap_enabled) {
			auto mipmap_tex = streamfx::obs::gs::texture::pool::instance()->acquire(_cache_size.first, _cache_size.second, GS_RGBA, static_cast<uint32_t>(_mip_levels), nullptr, streamfx::obs::gs::texture_flags::None);
			_mipmapper.rebuild(buffer_tex, mipmap_tex);
			buffer_tex = mipmap_tex;

			// 'cache_rt' is now no longer in use, so we can release it early.
			cache_rt.reset();
		}

		// Now finally apply the 3D Transform effect.
		{
			auto op = _final_rt->render(_source_size.first, _source_size.second);
			...
			case transform_mode::PERSPECTIVE:
				gs_perspective(_camera_fov, float(_source_size.first) / float(_source_size.second), nearZ, farZ);
	...
	{
		gs_effect_set_texture(gs_effect_get_param_by_name(effect, "image"), *_final_tex);
